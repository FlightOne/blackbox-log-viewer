...
var nwBuilderOptions = {
    ...
    macPlist: { 'CFBundleDisplayName': 'FlightOne Blackbox Explorer'},
...
function getReleaseFilename(platform, ext) {
    return 'flightone-blackbox-explorer_' + pkg.version + '_' + platform + '.' + ext;
}
...
function release_zip(arch) {
    ...
    return compressFiles(src, base, output, 'FlightOne Blackbox Explorer');
}
...
function release_deb(arch, done) {
    ...
    return gulp.src([path.join(APPS_DIR, pkg.name, arch, '*')])
        .pipe(deb({
             ...
             postinst: ['cp /opt/flightone/flightone-blackbox-explorer/mime/flightone-blackbox-explorer.xml /usr/share/mime/packages/;update-mime-database /usr/share/mime',
                        'cp /opt/flightone/flightone-blackbox-explorer/icon/bf_icon_128.png /usr/share/icons/hicolor/128x128/mimetypes/application-x-blackboxlog.png;gtk-update-icon-cache /usr/share/icons/hicolor -f',
                        'xdg-desktop-menu install /opt/flightone/flightone-blackbox-explorer/flightone-blackbox-explorer.desktop'],
             prerm: ['rm /usr/share/mime/packages/flightone-blackbox-explorer.xml;update-mime-database /usr/share/mime',
                     ...
                     'xdg-desktop-menu uninstall flightone-blackbox-explorer.desktop'],
             depends: 'libgconf-2-4',
             changelog: [],
             _target: 'opt/flightone/flightone-blackbox-explorer',
             _out: RELEASE_DIR,
    ...
    var options = {
             ...
             files:
                 [ { cwd: path.join(APPS_DIR, pkg.name, arch),
                     src: '*',
                     dest: '/opt/flightone/blackbox-log-viewer' } ],
             postInstallScript: ['cp /opt/flightone/flightone-blackbox-explorer/mime/flightone-blackbox-explorer.xml /usr/share/mime/packages/;update-mime-database /usr/share/mime',
                                 'cp /opt/flightone/flightone-blackbox-explorer/icon/bf_icon_128.png /usr/share/icons/hicolor/128x128/mimetypes/application-x-blackboxlog.png;gtk-update-icon-cache /usr/share/icons/hicolor -f',
                                 'xdg-desktop-menu install /opt/flightone/blackbox-log-viewer/blackbox-log-viewer.desktop'],
             preUninstallScript: ['rm /usr/share/mime/packages/flightone-blackbox-explorer.xml;update-mime-database /usr/share/mime',
                                  'rm /usr/share/icons/hicolor/128x128/mimetypes/application-x-blackboxlog.png;gtk-update-icon-cache /usr/share/icons/hicolor -f',
        ...
        .pipe(appdmg({
            ...
            specification: {
                ...
                contents: [
                    ...
                    { 'x': 192, 'y': 344, 'type': 'file', 'path': pkg.name + '.app', 'name': 'Flightone Blackbox Explorer.app' }
                ],
...
function listReleaseTasks(done) {
    ...
    if (platforms.indexOf('win32') !== -1) {
        releaseTasks.push(function release_win32_zip(){ return release_zip('win32') });
        releaseTasks.push(function release_win32(done){ return release_win('win32', done) });
    }

    if (platforms.indexOf('win64') !== -1) {
        releaseTasks.push(function release_win64_zip(){ return release_zip('win64') });
